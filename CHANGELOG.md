...
- **Coverage gate measures the whole product** — the 11 previously-excluded top-level orchestration modules are measured again (56,274 statements at HEAD); in-process dispatch tests, deep activation/workspace/setup/rebuild/discover pipeline tests (including the full workspace and rebuild lifecycles driven in-process with fetch mocks) brought the gate to 99.59% statements / 99.86% functions on the deterministic serial measurement. The remaining delta is precisely inventoried in `coverage/coverage-gaps.md`: a Windows-specific multi-process merge artifact class (function-name zero-ranges on executed lines; each provably executed via per-file c8 runs) plus a closed list of real defensive branches (CLI entry guards, doctor race fallbacks, breadth assessment outcomes). The coverage gate runs serialized (`--test-concurrency=1`) because parallel children race on the V8-coverage temp dir; CI (Linux) is the canonical arbiter for the final 1% (#428)
