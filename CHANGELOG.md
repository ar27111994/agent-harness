...
- **Quarantine dispatcher split** — the last ~600-line monolith from the split wave became a `src/quarantine/` domain: help text in `help.ts`, the state/report/review model in `state.ts`, and the dispatcher + command flows down to 230 lines (#435)
