...
- **Review wave (2026-08-11 from-scratch audit) closed**:
  ...
  - **Native install plans exclude single-token coincidence extensions with explicit status lines** — entries carry an optional `coincidentalMatchOnly` flag when their only declared-package matches are identity-denied tokens; `install native` excludes them from plan and apply/remove with a note naming the signal, and every kept extension prints its basis plus "native install via host CLI (not mirrored)" (#444 AC3 review)
