...
- **Review wave (2026-08-11 from-scratch audit) closed**:
  - **Install-domain host validation and bundle lookups print clean one-liners** — `install reconcile/reset/refresh --host` validation now throws the shared `CliUsageError` (one message shape, subcommand usage hint) instead of a raw stack, and `bundle explain <unknown-bundle>` reports "Bundle lock not found" with a recovery pointer instead of a bare ENOENT stack; spawned-CLI regression tests assert stack-free stderr on all four paths (review)
  - **Exact-stack requires ecosystem affinity for every source class** — the ecosystem gate and mismatch penalty now apply to non-package-registry assets published under an unambiguous source-family language (`wordpress`/`drupal`/`joomla` → php): an `official-index:WordPress` skill loses `fit:exact-stack` and takes the 2× penalty in a TypeScript workspace instead of ranking via the synonym-collapsed `api → node-backend` coincidence (#443 AC3 review)
  - **Declared-dependency identity matches outrank exact-stack tiles** — a signal whose demand term carries package identity and whose token appears in the asset's RAW curated identity (uncanonicalized: the synonym table maps `duckdb → data`, so canonical identities lost the token) receives the `identityMatchMultiplier` (policy, default 4); live repro: the official DuckDB skill moved from opencode rank 25 to rank 11 (#443 AC2 review)
  - **Native install plans exclude single-token coincidence extensions with explicit status lines** — entries carry the optional `coincidentalMatchOnly` flag (included only when true; consumers treat an absent flag as `false`) when their only declared-package matches are identity-denied tokens; `install native` excludes them from plan and apply/remove with a note naming the signal, and every kept extension prints its basis plus "native install via host CLI (not mirrored)" (#444 AC3 review)
  - **All-axes suite hardening** — BOM stripping (a BOM'd fetched feed used to silently sync nothing) + duplicate-key/`__proto__` semantics pinned; 0-byte/255-char/whitespace/null-byte/extension-less file-shape battery; Codex added to cli-smoke and workspace-smoke (was absent from every CLI-level suite); cli-smoke asserts output + clean stderr; e2e wire round-trip asserts stack-free re-apply/reset with no managed leftovers; pack-smoke inspects tarball contents against the single source of truth; detection quality gains per-fixture floors; perf-path gains a 10,000-entry scale case with a super-linearity guard (review)
...
- **Codex marketplace ownership survives an unchanged reapply** — a second apply recomputed whole-file ownership from the file's mere existence, flipping a harness-created marketplace to `false` and stranding the leftover empty file on reset; the ownership record now preserves a prior `created: true` across reapplies (review)
- **Codex passing a smaller agent set reconciles dropped profiles** — an apply with a subset of the previous agents replaced the ownership manifest with only the incoming records, orphaning dropped agents' generated TOML profiles (and their displaced user content) in `.codex/agents`; reapply now removes harness-created profiles and restores displaced user content before rewriting the manifest (review)
- **`check:version-sync` became a `guardrails` gate and allows a forward-bump** — the tag-vs-manifest guard (#467) now runs first inside `validate`, rejects a version that regresses below the highest released tag, and treats a bumped version awaiting its post-merge tag as valid (the tag is created on main after merge; the release workflow enforces it for tagged runs)
...
- **Native-command injection tests** — strict VS Code extension-id boundary battery plus byte-exact hostile-argv round-trips; on Windows a real `.cmd` wrapper proves safe arguments survive PowerShell quoting verbatim while metacharacter arguments are refused before any shell sees them (#428 hardening)
- **RTL/unicode path suite** — dedicated tests drive Arabic, Hebrew, CJK, combining marks, zero-width/bidi-format characters, emoji, and full-width confusables through safe-path resolution, mirror/asset id sanitization, posix conversion, lifecycle fingerprints (punycode-folded non-ASCII URL hostnames, verbatim non-URL unicode identity), report grouping with unicode duplicate ids, and real filesystem round-trips through RTL directory names; the contract asserted is that unicode is data, never a traversal bypass (#428 hardening)
- **Quarantine size-boundary suite** — pins the stars popularity cut-off (100 vs 101), the 0..100 score clamp, the exact 40/70 decision boundaries, risky-kind and risk-flag overrides, the duplicate-group −10 penalty, the acquisition status precedence (prompt injection, executable/community/high risk, reference-only, official warning vs community quarantine), the NFKC confusable detection for prompt injection (full-width, mathematical-bold, bidi-wrapped, patterns spread across materialized files) with no false positives on innocent international text, and byte-exact preservation of a 1.5 MiB quarantined artifact (#428 hardening)
- **Duplicate-id edge suite** — singleton groups never surface as duplicate decisions but keep their identity tag; group members, evidence, and fingerprint lists are deterministically sorted; unmirrored members expose absent content hashes honestly; duplicate catalog ids are preserved rather than de-duplicated; cross-source duplicates converge on one canonical upstream identity (hostname casing, terminal `.git` folding) while source-scoped identity stays distinct; and the in-process select pipeline records decisions only for real groups and rejects exactly the duplicate loser (#428 hardening)
...
- **Review-2026-08-09 gap wave** — the from-scratch review verdict was READY with a defined minor set, now closed:
  ...
  - **Committed stress fixtures**: high-load atomic-churn stress (16 writers × 20 rounds, 8 KiB payloads, 8 readers), three-contender seed-lock serialization (sections never overlap under the O_EXCL polling race), and the exact adversarial path shapes from the CodeQL js/polynomial-redos alert notes against the segment-scan classifier (deep `./rules/` repetitions, 300-deep plugins chains, 100k-char segments)
  - **DRY + #433 shape**: the cargo/pyproject/pubspec extractors share one `walkSectionedLines` iterator (configurable comment stripping + column-0 header requirement), and all regex-group `match[N]!` assertions are replaced by the guarded `requiredCaptureGroup` with descriptive errors (throw arms covered directly)
  - **#428 AC letter**: the gate tools' own c8-ignore comments are gone — the direct-execution guards of `check-coverage-exclusions.mjs` and `check-env-readme-drift.mjs` are now covered by spawned direct runs merged back into the gate; `maintenance-bot-plan.mjs` keeps its documented pre-existing guard (direct run needs the live bot-plan environment)
  - **Metadata/docs hygiene**: `docs/README.md` indexes `cheatsheet.md` and `ADAPTER-DEVELOPMENT.md`; `SEMANTIC-SCORING.md` drops the dead `--no-semantic` future-flag sentence (the flag hard-errors under #431 strict validation); the four stale pre-wave audit reports were relocated to the QA reports dir; the stray PowerShell module-analysis cache is deleted and its pattern gitignored. The npm keywords were evaluated against the pruning suggestion and KEPT: the package-metadata contract test pins them as the handled asset surface (the harness wires and mirrors MCP-server assets, shared-MCP flows, and registry queries), while the #425 role-overclaim fix lives in the description and AGENTS.md
- **Polynomial-ReDoS paths hardened (CodeQL high-severity)** — the first code-scanning run on the PR flagged seven `js/polynomial-redos` alerts: the Cursor-config classifiers in `local-harvesters.ts` used nested `.+` quantifiers (`^(?:plugins\/.+\/)?rules\/.+\.[mc]d$` etc.) that backtrack polynomially on adversarial relative paths, and `sanitizeAssetId` / `normalizeSessionIntentValue` used `+` over negated character classes on untrusted input. The classifiers now use the unambiguous `(?:[^/]+/)*` level-delimited form (identical accepted-language, linear matching), and the run-collapse logic is a shared one-pass `replaceRunsWithDash` scan (no regex backtracking at all)
- **Cross-platform coverage gaps closed for the CI matrix** — the first 3-OS gate run revealed platform-isolated branches the Windows-only local gate never exercised: the fail-closed Windows-shell-wrapper refusals in `preflight.ts` (`runRuntimeCommand`) and `extension-installer.ts` (`executeNativeCommand`) only execute on win32, and the discover `--summary` already-seen-reason map branch needed a duplicated warning reason. The refusal decisions are extracted into platform-parameterized helpers (`buildWrapperRefusal` / `buildShellWrapperRefusal`, exported via the internals seams) so tests force the win32 wrapper branch on any runner; the summary test seeds two failed sources sharing one reason. The coverage gate now passes on ubuntu/macos/windows, not just the dev machine
- **Workspace/doctor test fixtures are hermetic for host config dirs** — the first CI run (this branch's PR) exposed six latent platform/isolation failures: the fixtures resolved host config dirs from the real machine's environment, so `setup doctor` / `wire` failed with `opencode-config-directory`/`vscode-user-settings-directory` errors on clean runners (they only passed locally because the dev machine happens to have an opencode config dir and the opencode CLI installed, which masked the check by creating the dir on probe), and the workspace pipeline tests seeded only `catalog.assets.jsonl` — which the pipeline genuinely regenerates from discovery state — so the recommend phase found zero entries on machines without real host config dirs. The shared fixtures now redirect `AGENT_HARNESS_HOME`/`APPDATA`/`HOME`/`USERPROFILE`/`XDG_CONFIG_HOME` into the isolated temp root (with config-cache reset, restoring via `restoreEnvVar`), materialize the platform-appropriate opencode and VS Code config roots, and the pipeline tests seed an indexed source + sync state + entry artifacts so catalog generation is deterministic. Verified locally with the real config dirs absent: 41/41 in the affected files, full suite green
- **CI coverage gate runs on Node 24** — the quality and maintenance workflows pinned Node 22, but the gate's `--test-isolation=none` (single-process isolation, load-bearing for the honest 100% claim) ships with Node 23, so the first PR run failed with `node: bad option: --test-isolation=none` (exit 9). The workflows now use Node 24 — matching `release.yml` and the documented floor — and `engines`/README/badge were corrected from `>=22` to `>=23` with the reason stated
- **Atomic readers retry the Windows-transient open window** (#427/#428) — the atomic writer already retried EPERM/EACCES during its remove-and-rename window, but a concurrent reader opening the same destination could hit the same momentary lock (AV scan, open handle, a competing writer's rename) and surface a transient error — violating the churn contract ("old | absent | complete-new, never a transient open error") that the cross-process contention suite asserts. `readJsonFile`, `readTextFileOrNull`, and `readBinaryFileOrNull` now retry the identical bounded window (EPERM/EACCES only; ENOENT is still the contract's "absent" and is never retried), with a `filesInternals.setReadFileOpenOverride` failure-injection seam and unit tests for retry-then-success, exhaustion, and fail-fast paths
- **Package-audit fixtures kept in sync with the pinned required-set** — the tarball pin added the split-domain runtime files (`dist/activate/{help,selection,state}.js`, `dist/quarantine/{help,state}.js`) to `REQUIRED_PACKED_FILES` but the fixture in `package-audit.test.mjs` kept its own copy of the old list, so every fixture-based audit test failed deterministically (`Missing required files: dist/activate/help.js, ...`) and the direct-execution / missing-trust-docs assertions matched the wrong message. The fixture now imports the exported required set — one source of truth for checker and check (red-gate at HEAD, caught by the closeout suite re-run)
- **Environment restore no longer leaks the literal string `"undefined"` into the suite** — restoring a previously-unset variable with `process.env.X = previousX` coerces to `"undefined"` (Node semantics), and the coverage gate's `--test-isolation=none` carried that string across test files and into every spawned child: the e2e CLI children handed `XDG_CONFIG_HOME="undefined"` / `AGENT_HARNESS_HOME="undefined"` to the setup-doctor host probes, and the installed opencode CLI resolved its config dir from them, creating a stray `undefined/opencode` directory in the repo root (the review's cosmetic stray dir was a symptom, not the cause). `discover-breadth.test.ts` — the only unguarded site — and `github-catch-coverage.test.ts` now restore through the shared delete-guarded `restoreEnvVar` helper (`src/tests/env-test-utils.ts`), and every other inline restore in the suite (30 files, 74 sites) was migrated to the same helper. Verified: the discovery+e2e pair no longer creates the directory, and the full suite passes without resurrecting it
- **Coverage gate restored at HEAD** (#428) — the v2.1.0 refactor wave left the repo's own gate red: 66 public exports of the new split modules lacked docstrings and the cli.ts help-dispatch switch plus the install/quarantine/rebuild/setup subcommand-help printers sat outside the measured set (spawned CLI children strip `NODE_V8_COVERAGE` by design). All 66 docstrings are restored and the help surface is now covered in-process, returning the gate to a verified 0/0/0/0 uncovered statements/branches/functions/lines.
- **State-root seeding serialized across concurrent CLI processes** (#428 hardening) — two first runs against the same fresh state root raced `prepareStateRoot`'s managed-asset copy (one crashed with EEXIST, or worse, one process's clean deleted a tree the other had just populated). Seeding now runs under an exclusive per-root lock file (`O_EXCL`) with bounded waits, stale-lock recovery, and a clear timeout error naming the lock path.
- **Windows host wrapper commands fail closed on shell metacharacters** (#428 hardening) — cmd.exe re-parses the raw command line of a `.cmd`/`.bat` invocation, so PowerShell single-quoting could not protect arguments containing operators, redirection, or `%VAR%` expansion (a test proved `a&b` broke out and attempted to execute `b`). Wrapper targets now refuse any argument containing cmd metacharacters before a shell is involved; the VS Code extension installer applies the same guard on its shell-backed candidate path. Safe arguments (static literals, filtered extension ids) round-trip unchanged.
...
- **Coverage gate measures the whole product — 100%** — the 11 previously-excluded top-level orchestration modules are measured again (56,391 statements at HEAD). The coverage gate now runs **single-process** (`--test-isolation=none`), which eliminates the Windows multi-process merge artifact class (duplicate per-line/column DA records that fabricated false zeros on provably-executed lines and defeated `c8 ignore` suppression — verified with per-file single-process runs and a minimal instrumenter probe). The suite was made single-process-safe: adapter-registry snapshot/restore, script argv defaults gated on direct-entry execution, per-test cold caches for module-level resolution maps, load-generator semantics for the writer churn stresses, and proven-dead defensive branches removed outright. The gate post-processes the merged lcov through `scripts/normalize-lcov.mjs` (canonical max-aggregation of duplicate records) and enforces the hard 100% contract via `scripts/assert-full-coverage.mjs`; `coverage/coverage-gaps.md` is regenerated from the normalized file. Every in-process gap on the fresh gap map is closed with real unit/integration/e2e tests — sanitization, concurrency, and stress axes included (#428)
...
- **Recommend ranking 17x faster** — a CPU profile of the ranking/selection loop showed ~80% of wall time in phrase canonicalization: `shouldEnforceConcernTarget` rescanned every policy synonym per candidate per selection round, and `normalizePhrase` re-ran three regex passes per term. The enforced-concern set (invariant across the loop) is now precomputed once per host, and the pure normalization is memoized; a 1,000-entry recommendation report went from ~16.3s to ~0.97s on the reference machine with identical golden-fixture output (#428 hardening, `npm run benchmark:paths`)
- **Quarantine dispatcher split** — the last ~600-line monolith from the split wave became a `src/quarantine/` domain: help text in `help.ts`, the state/report/review model in `state.ts`, and the dispatcher + command flows down to 224 lines (#435)
- **CLI dispatch boilerplate consolidated** — the identical unknown-command guard in all seven domain dispatchers is one `handleUnknownCommand` helper, setup's repeated unknown-flag guards are a flag-spec table, `rejectUnknownFlags` is renamed to the predicate `hasUnknownFlag`, and the setup/workspace preflight injection seams share one `PreflightFunctionOverrides` type (standards axis)
- **Self-hosting suite budget raised to 450s** — the full-pipeline self-host test measured ~5m40s on Windows, over the old 300s budget; TROUBLESHOOTING documents the honest runtime and CI headroom (standards axis)
- **Module decomposition** — `discover.ts` (2,003 → 619 total / 539 real-logic lines, under the #434 ~600 real-logic budget) split into `discover-help.ts` (help copy) and `discover-pipeline.ts` (breadth assessment, semantic relevance filtering, demand-relevant source selection); `demand-signals.ts` (1,364) split into `demand-dependency-extractors.ts` (397 total / 264 logic), `demand-file-classification.ts` (274 total / 195 logic), and `demand-manifest-enrichment.ts` (674 total / 507 logic), leaving a 336-line coordinator; `activate.ts` (1,149) became a 507-line dispatcher plus the `src/activate/` domain (`help.ts` 119, `selection.ts` 422 total / 282 logic, `state.ts` 326 total / 240 logic); `recommend/counts.ts` middle-man wrappers collapsed to `countCoverageTagsForItems`; `mirror/plan.ts` reads discovery artifacts through the shared typed `loadDiscoveryArtifacts` loader; the duplicated test-framework filename check in `demand-signals.ts` is one `isTestFrameworkConfigFileName` helper (#434, #435, #436, #437, #438)
- **no-magic-numbers guard re-pointed** — the CONTRIBUTING-documented rule's file list drifted during the split wave: it still named the pre-split homes (`src/activate.ts`, `src/domains/discovery/demand-signals.ts`) while the policy thresholds moved into `src/activate/selection.ts`, `src/activate/state.ts`, and the demand-dependency/file-classification/manifest-enrichment modules, vacating the enforcement for the code it polices. The list now follows the constants (verified live: a bare return literal in `src/activate/state.ts` is reported, the same literal in an unguarded module is not) (standards axis, review finding)
