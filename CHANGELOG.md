...
- **Review-2026-08-09 gap wave** — the from-scratch review verdict was READY with a defined minor set, now closed:
  ...
  - **Metadata/docs hygiene**: npm keywords no longer advertise `mcp-server`/`mcp-manager`/`model-context-protocol`; `docs/README.md` indexes `cheatsheet.md` and `ADAPTER-DEVELOPMENT.md`; `SEMANTIC-SCORING.md` drops the dead `--no-semantic` future-flag sentence (the flag hard-errors under #431 strict validation); the four stale pre-wave audit reports were relocated to the QA reports dir; the stray PowerShell module-analysis cache is deleted and its pattern gitignored
