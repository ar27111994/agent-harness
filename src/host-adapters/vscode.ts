...
async function materializeWorkspaceInstructions(
  ...
  for (const instructionId of profileManifest?.selectedInstructionIds ?? []) {
    ...
    bodyLines.push(`<!-- ${instructionId} -->`);
    bodyLines.push(resolvedAsset.content.trim());
    bodyLines.push("");
  }

  const existingContent = (await readTextFileOrNull(destinationPath)) ?? "";
  await writeTextFile(
    destinationPath,
    upsertManagedSection({
      originalContent: existingContent,
      markerId: "agent-harness-vscode",
      bodyLines,
    }),
  );
}
...
async function removeManagedWorkspaceInstructions(
