...
void test("source health report distinguishes active, dormant, stale, failed, and trust drift", () => {
  ...
  assert.equal(report.severeCount, 2);
...
function sourceStatus(
  ...
  return report.sources.find((source) => source.sourceId === sourceId)?.status;
}
...
function buildEntry(id: string, sourceId: string): AssetCatalogEntry {
  return {
    id,
    displayName: id,
    assetKind: "skill",
